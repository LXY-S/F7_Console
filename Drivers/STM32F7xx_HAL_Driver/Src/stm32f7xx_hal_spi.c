...
@verbatim
 ...
 ===============================================================================
    ...
    This subsection provides a set of functions allowing to control the SPI.
     (+) HAL_SPI_GetState() API can be helpful to check in run-step the state of the SPI peripheral
     (+) HAL_SPI_GetError() check in run-step Errors occurring during communication
@endverbatim
  * @{
  */
...
static HAL_StatusTypeDef SPI_EndRxTransaction(SPI_HandleTypeDef *hspi,  uint32_t Timeout, uint32_t Tickstart)
{
  ...
  else /* SPI_MODE_SLAVE */
  {
    ...
    /* Wait BSY flag during 1 Byte step transfer in case of Rx transfer
    * If Timeout is reached, the transfer is considered as finish.
    * User have to calculate the timeout value to fit with the step of 1 byte transfer.
    * This step is directly link with the SPI clock from Master device.
    */
    do
    {
      if (count == 0U)
...
static HAL_StatusTypeDef SPI_EndRxTxTransaction(SPI_HandleTypeDef *hspi, uint32_t Timeout, uint32_t Tickstart)
{
  ...
  {
    /* Wait BSY flag during 1 Byte step transfer in case of Full-Duplex and Tx transfer
    * If Timeout is reached, the transfer is considered as finish.
    * User have to calculate the timeout value to fit with the step of 1 byte transfer.
    * This step is directly link with the SPI clock from Master device.
    */
